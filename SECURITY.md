...
1. Use GitHub's **private vulnerability reporting** on the
   [Security tab](https://github.com/PerryLink/dsh-github/security) of this repository
...
**Sanitize before reporting:** redact all tokens, keys, request headers, and other
credentials from logs, screenshots, and reproduction steps. The plugin's display and
log layers redact credentials, but your report is a new channel — re-check it yourself.
...
This policy covers the `dsh-github` repository and the npm package `@perrylink/dsh-github`.
...
The latest published release (currently the 0.6.x line) receives security fixes.
