    ...
    return commands.register({
        ...
        handler: async (invocation) => {
            ...
            if (input.startsWith('post ')) {
                ...
                notify(invocation.agent, `The user ran /review post ${jobId}. Call the review_post tool with { "jobId": "${jobId}" } to publish the review `
...
function parseReviewOptions(input) {
    const parts = input.split(/\s+/);
    const options = { includeCi: true, includeComments: true };
    let pr = '';
    for (let index = 0; index < parts.length; index += 1) {
        ...
        if (part === '--max-diff') {
            ...
            if (value === undefined || !/^\d+$/.test(value))
                throw new Error(`--max-diff needs a positive number. ${USAGE_REVIEW}`);
        ...
        if (pr.length > 0)
            throw new Error(`unexpected argument "${part}". ${USAGE_REVIEW}`);
        pr = part;
    }
    if (pr.length === 0)
        throw new Error(USAGE_REVIEW);
    return { pr, options };
}
