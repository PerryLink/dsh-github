...
export function analyzeDiff(diff, maxChars) {
    ...
    for (const [file, count] of perFileAdded) {
        if (findings.length >= MAX_FINDINGS)
            break;
        if (count > 400)
            findings.push({ file, line: null, severity: 'info', rule: 'large-change', message: `本 PR 为该文件新增 ${count} 行，建议拆分提交` });
    }
...
export function formatPostBody(findings, truncated) {
    ...
    if (truncated)
        sections.push('> 注：diff 超过上限已截断，本报告只覆盖截断范围内的内容。');
    sections.push('');
