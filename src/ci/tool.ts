...
const TASKS: CiTask[] = ['review', 'analyze', 'publish']

/** `ci_run`: run one CI review pass (write; approval-gated upstream). */
export function ciRunTool(state: GithubState) {
  return defineTool({
    name: 'ci_run',
    ...
    output: {
      ...
      render: (args, value) => {
        ...
        if (value.review !== undefined && value.review.url.length > 0) lines.push(`review: ${value.review.url}`)
        if (value.files !== undefined) lines.push(`reports: ${value.files.json} · ${value.files.markdown}`)
        if (value.diffText !== undefined) lines.push(`diff (${value.diffText.length} chars${value.truncated ? ', truncated' : ''}):\n${value.diffText}`)
