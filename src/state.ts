 ...
 * @param ctx - context holding the credentials seam.
 ...
 */
export function createState(ctx: { credentials: CredentialProvider }, config: Config, runGit: GitRunner, runGh: GhRunner, fetchImpl?: typeof fetch): GithubState {
...
async function runGitRemote(this: void, state: RepoStateView, cwd: string, signal?: AbortSignal): Promise<{ repoFromRemote: string | null }> {
  try {
    ...
    return { repoFromRemote: repoFromRemoteUrl(stdout) }
  } catch {
    return { repoFromRemote: null }
  }
}

/** Shared shape of rate-limit facts on read-tool results. */
