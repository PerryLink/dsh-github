...
export function startReviewJob(registry: JobRegistry, state: GithubState, input: StartReviewJobInput): GithubJobId {
  if (state.config.reviewMode === 'model' && state.subagents === undefined) {
    throw new Error(
      'reviewMode "model" requires the subagent seam: load @deepseek-ai/dsh-subagent with an in-process provider '
...
function runReviewWork(state: GithubState, record: ReviewJobRecord, input: Required<Pick<StartReviewJobInput, 'timeoutMs' | 'maxDiffChars' | 'includeCi' | 'includeComments'>> & Pick<StartReviewJobInput, 'owner'>): { cancel: (reason?: string) => void; done: Promise<JobOutcome> } {
  ...
  const done = (async (): Promise<JobOutcome> => {
    try {
      ...
      const metadata = await client.requestJson<PullMetadataPayload>('GET', `/repos/${record.repo}/pulls/${record.pr}`, { signal: controller.signal })
      ...
      if (input.includeCi) {
        try {
          const ci = await client.requestJson<CheckRunsPayload>('GET', `/repos/${record.repo}/commits/${record.headSha ?? ''}/check-runs`, { signal: controller.signal })
      ...
      if (input.includeComments) {
        try {
          const comments = await client.requestJson<unknown[]>('GET', `/repos/${record.repo}/pulls/${record.pr}/comments?per_page=${state.config.maxComments}`, { signal: controller.signal })
...
function summarizeChecks(runs: Array<{ name?: string; status?: string; conclusion?: string | null }>): string {
