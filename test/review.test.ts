...
describe('analyzeDiff', () => {
  ...
  it('caps the diff and reports truncation', () => {
    ...
    expect(report.summary).toContain('diff truncated')
  ...
  it('formats a postable Markdown body grouped by file', () => {
    ...
    expect(body).toContain('## dsh-github review')
    expect(body).toContain('### `src/app.ts`')
    expect(body).toContain('hardcoded-secret')
    expect(body).toContain('*Generated by [dsh-github]')
  })
  ...
  it('reports clean diffs without findings', () => {
    ...
    expect(report.summary).toContain('no obvious issues found')
  })

  it('flags Google API keys and credential assignments', () => {
    const diff = 'diff --git a/c.ts b/c.ts\n--- a/c.ts\n+++ b/c.ts\n@@ -1 +1,2 @@\n x\n+const key = "AIzaSyD4iE2xVh2aB0C1dE3fG4hI5jK6lM7nO8pQ9"\n+const password = "hunter2secret"\n'
    const report = analyzeDiff(diff, 100_000)
    const rules = report.findings.map(finding => finding.rule)
    expect(rules).toContain('google-api-key')
    expect(rules).toContain('hardcoded-credential')
  })

  it('records the truncation fact on the report', () => {
    const report = analyzeDiff(SAMPLE_DIFF, 40)
    expect(report.truncated).toBe(true)
    const whole = analyzeDiff(SAMPLE_DIFF, 100_000)
    expect(whole.truncated).toBe(false)
  })

  it('escapes hostile file names in the posted body', () => {
    const findings = [
      { file: 'bad`file<script>.ts', line: 3, severity: 'info' as const, rule: 'todo-marker', message: 'leftover marker' },
      { file: 'evil\nname.ts', line: null, severity: 'warning' as const, rule: 'large-change', message: 'too large' },
    ]
    const body = formatPostBody(findings, false)
    expect(body).toContain('`bad\\`file&lt;script&gt;.ts`')
